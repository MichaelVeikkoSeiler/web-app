...
type ExistingCheck = Awaited<ReturnType<typeof findExistingPlant>>;

type Step =
  | { name: "capture" }
  | { name: "identifying" }
  | { name: "candidates"; candidates: PlantNetCandidate[]; error?: string }
  | { name: "checking"; candidate: PlantNetCandidate }
  | { name: "existing"; candidate: PlantNetCandidate; existing: NonNullable<ExistingCheck> }
...
export function NewPlantWizard({
  ...
  async function finishWithPhoto(plantId: number) {
    if (photoFile) {
      try {
        const url = await uploadPlantPhoto(photoFile);
        await savePlantPhoto(plantId, url, true);
      } catch {
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col gap-6">
      <h1 className="font-display text-2xl text-forest">Pflanze hinzufügen</h1>
      ...
      {step.name === "checking" && (
        <StatusBox icon={<Loader2 className="h-5 w-5 animate-spin" />} text="Prüfe, ob die Pflanze schon erfasst ist…" />
      )}
      ...
      {!showZonePicker ? (
        <div className="flex flex-col gap-2 pt-2">
          <Button onClick={onSameZone}>
            <Check className="h-4 w-4" /> Dieselbe Pflanze, gleiche Zone
          </Button>
...
function NewPlantZoneStep({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-border bg-warm-white p-4">
      <p className="text-sm text-forest">
        Neue Pflanze: <span className="font-medium italic">{candidate.scientificName}</span>
      </p>
      {zones.length === 0 ? (
        <p className="text-sm text-attention-text">
          Bitte zuerst unter „Zonen“ mindestens eine Zone anlegen.
        </p>
      ) : (
        <>
